...
#    MySui Online Judge
#    Copyright (C) 2014  MySui Team <mysuioj@gmail.com>
#
...
msoj_log "Starting tester..."

# detecting existence of perl
PERL_EXISTS=true
hash perl 2>/dev/null || PERL_EXISTS=false
if ! $PERL_EXISTS; then
	msoj_log "Warning: perl not found. We continue without perl..."
fi
...
JAIL=jail-$RANDOM
if ! mkdir $JAIL; then
	msoj_log "Error: Folder 'tester' is not writable! Exiting..."
	msoj_finish "Judge Error"
fi
cd $JAIL
cp ../timeout ./timeout
chmod +x timeout
...
msoj_log "$(date)"
msoj_log "Language: $EXT"
msoj_log "Time Limit: $TIMELIMIT s"
msoj_log "Memory Limit: $MEMLIMIT kB"
msoj_log "Output size limit: $OUTLIMIT bytes"
if [[ $EXT = "c" || $EXT = "cpp" ]]; then
	msoj_log "EasySandbox: $SANDBOX_ON"
	msoj_log "C/C++ Shield: $C_SHIELD_ON"
elif [[ $EXT = "py2" || $EXT = "py3" ]]; then
	msoj_log "Python Shield: $PY_SHIELD_ON"
elif [[ $EXT = "java" ]]; then
	msoj_log "JAVA_POLICY: \"$JAVA_POLICY\""
	msoj_log "DISPLAY_JAVA_EXCEPTION_ON: $DISPLAY_JAVA_EXCEPTION_ON"
fi
...
if [ "$EXT" = "java" ]; then
	...
	msoj_log "Compiling as Java"
	javac $MAINFILENAME.java >/dev/null 2>cerr
	EXITCODE=$?
	COMPILE_END_TIME=$(($(date +%s%N)/1000000));
	msoj_log "Compiled. Exit Code=$EXITCODE  Execution Time: $((COMPILE_END_TIME-COMPILE_BEGIN_TIME)) ms"
	if [ $EXITCODE -ne 0 ]; then
		msoj_log "Compile Error"
		msoj_log "$(cat cerr|head -10)"
		echo '<span class="msoj_b">Compile Error</span>' >$PROBLEMPATH/$UN/result.html
		echo '<span class="msoj_r">' >> $PROBLEMPATH/$UN/result.html
...
if [ "$EXT" = "py2" ]; then
	...
	msoj_log "Checking Python Syntax"
	python2 -O -m py_compile $FILENAME.py >/dev/null 2>cerr
	EXITCODE=$?
	COMPILE_END_TIME=$(($(date +%s%N)/1000000));
	msoj_log "Syntax checked. Exit Code=$EXITCODE  Execution Time: $((COMPILE_END_TIME-COMPILE_BEGIN_TIME)) ms"
	if [ $EXITCODE -ne 0 ]; then
		msoj_log "Syntax Error"
		msoj_log "$(cat cerr | head -10)"
		echo '<span class="msoj_b">Syntax Error</span>' >$PROBLEMPATH/$UN/result.html
		echo '<span class="msoj_r">' >> $PROBLEMPATH/$UN/result.html
		(cat cerr | head -10 | sed 's/&/\&amp;/g' | sed 's/</\&lt;/g' | sed 's/>/\&gt;/g' | sed 's/"/\&quot;/g') >> $PROBLEMPATH/$UN/result.html
		echo "</span>" >> $PROBLEMPATH/$UN/result.html
		cd ..
		rm -r $JAIL >/dev/null 2>/dev/null
		msoj_finish "Syntax Error"
	fi
	if $PY_SHIELD_ON; then
		msoj_log "Enabling Shield For Python 2"
...
if [ "$EXT" = "py3" ]; then
	...
	if $PY_SHIELD_ON; then
		msoj_log "Enabling Shield For Python 3"
		# adding shield to beginning of code:
...
if [ "$EXT" = "c" ] || [ "$EXT" = "cpp" ]; then
	...
	msoj_log "Compiling as $EXT"
	if $SANDBOX_ON; then
		msoj_log "Enabling EasySandbox"
		if cp ../easysandbox/EasySandbox.so EasySandbox.so; then
			chmod +x EasySandbox.so
		else
			msoj_log 'EasySandbox is not built. Disabling EasySandbox...'
			SANDBOX_ON=false
		fi
	fi
	if $C_SHIELD_ON; then
		msoj_log "Enabling Shield For C/C++"
	...
	if [ $EXITCODE -ne 0 ]; then
		msoj_log "Compile Error"
		msoj_log "$(cat cerr | head -10)"
		echo '<span class="msoj_b">Compile Error<br>Error Messages: (line numbers are not correct)</span>' >$PROBLEMPATH/$UN/result.html
		echo '<span class="msoj_r">' >> $PROBLEMPATH/$UN/result.html
		SHIELD_ACT=false
		if $C_SHIELD_ON; then
			while read line; do
...
if [ -f "$PROBLEMPATH/tester.cpp" ] && [ ! -f "$PROBLEMPATH/tester.executable" ]; then
	...
	if [ $EC -ne 0 ]; then
		msoj_log "Compiling tester failed."
		cd ..
		rm -r $JAIL >/dev/null 2>/dev/null
		msoj_finish "Invalid Tester Code"
	else
		msoj_log "Tester compiled. Execution Time: $((TST_COMPILE_END_TIME-TST_COMPILE_BEGIN_TIME)) ms"
	fi
fi

if [ -f "$PROBLEMPATH/tester.executable" ]; then
	msoj_log "Copying tester executable to current directory"
	cp $PROBLEMPATH/tester.executable msoj_tester
	chmod +x msoj_tester
fi
...
for((i=1;i<=TST;i++)); do
	msoj_log "\n=== TEST $i ==="
	echo "<span class=\"msoj_b\">Test $i</span>" >>$PROBLEMPATH/$UN/result.html
	...
	if [ "$EXT" = "java" ]; then
		...
		if grep -iq -m 1 "Too small initial heap" out || grep -q -m 1 "java.lang.OutOfMemoryError" err; then
			msoj_log "Memory Limit Exceeded"
			echo "<span class=\"msoj_o\">Memory Limit Exceeded</span>" >>$PROBLEMPATH/$UN/result.html
			continue
		fi
		if grep -q -m 1 "Exception in" err; then # show Exception
			...
			msoj_log "Exception: $javaexceptionname\nMaybe at:$javaexceptionplace"
			...
			if $DISPLAY_JAVA_EXCEPTION_ON && grep -q -m 1 "^$javaexceptionname\$" ../java_exceptions_list; then
				echo "<span class=\"msoj_o\">Runtime Error ($javaexceptionname)</span>" >>$PROBLEMPATH/$UN/result.html
			else
				echo "<span class=\"msoj_o\">Runtime Error</span>" >>$PROBLEMPATH/$UN/result.html
			fi
			continue
		fi
		...
		msoj_log "File Format Not Supported"
		cd ..
		rm -r $JAIL >/dev/null 2>/dev/null
		msoj_finish "File Format Not Supported"
	fi

	msoj_log "Exit Code = $EXITCODE"

	if ! grep -q "FINISHED" err; then
		if grep -q "MSOJ_TIME" err; then
			t=`grep "MSOJ_TIME" err|cut -d" " -f3`
			msoj_log "Time Limit Exceeded ($t s)"
			echo "<span class=\"msoj_o\">Time Limit Exceeded</span>" >>$PROBLEMPATH/$UN/result.html
			continue
		elif grep -q "MSOJ_MEM" err; then
			msoj_log "Memory Limit Exceeded"
			echo "<span class=\"msoj_o\">Memory Limit Exceeded</span>" >>$PROBLEMPATH/$UN/result.html
			continue
		elif grep -q "MSOJ_HANGUP" err; then
			msoj_log "Hang Up"
			echo "<span class=\"msoj_o\">Process hanged up</span>" >>$PROBLEMPATH/$UN/result.html
			continue
		elif grep -q "MSOJ_SIGNAL" err; then
			msoj_log "Killed by a signal"
			echo "<span class=\"msoj_o\">Killed by a signal</span>" >>$PROBLEMPATH/$UN/result.html
			continue
		elif grep -q "MSOJ_OUTSIZE" err; then
			msoj_log "Output Size Limit Exceeded"
			echo "<span class=\"msoj_o\">Output Size Limit Exceeded</span>" >>$PROBLEMPATH/$UN/result.html
			continue
		fi
	else
	...
	if [ $EXITCODE -eq 137 ]; then
		#msoj_log "Time Limit Exceeded (Exit code=$EXITCODE)"
		#echo "<span style='color: orange;'>Time Limit Exceeded</span>" >>$PROBLEMPATH/$UN/result.html
		msoj_log "Killed"
		echo "<span class=\"msoj_o\">Killed</span>" >>$PROBLEMPATH/$UN/result.html
		continue
	fi


	if [ $EXITCODE -ne 0 ]; then
		msoj_log "Runtime Error"
		echo "<span class=\"msoj_o\">Runtime Error</span>" >>$PROBLEMPATH/$UN/result.html
		continue
	fi
	...
	ACCEPTED=false
	if [ -f msoj_tester ]; then
		./msoj_tester $PROBLEMPATH/in/input$i.txt $PROBLEMPATH/out/output$i.txt out
		EC=$?
		if [ $EC -eq 0 ]; then
			ACCEPTED=true
		fi
	...
	if $ACCEPTED; then
		msoj_log "ACCEPTED"
		echo "<span class=\"msoj_g\">ACCEPT</span>" >>$PROBLEMPATH/$UN/result.html
		((PASSEDTESTS=PASSEDTESTS+1))
	else
		msoj_log "WRONG"
		echo "<span class=\"msoj_r\">WRONG</span>" >>$PROBLEMPATH/$UN/result.html
	fi
done
...
# show place of exception. So I commented following lines:
	...
	#	echo -e "\n<span class=\"msoj_b\">Last Java Exception:</span>" >>$PROBLEMPATH/$UN/result.html
